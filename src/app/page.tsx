...
export default function Home() {
  ...
  const [mobilePane, setMobilePane] = useState<"signals" | "map" | "intel" | "warroom">("signals");
  ...
  /** Desktop: map is always visible behind sidebars — show reset chrome regardless of mobilePane. */
  ...
  const showMapResetButton = isMdUp || mobilePane === "map";
  ...
  return (
    <>
      ...
      <aside className={`fixed top-[36px] bottom-[48px] md:bottom-[28px] left-0 w-full md:w-[280px] z-40 bg-white/95 backdrop-blur-md border-r border-[#e4e4e7] transition-transform duration-300 ${mobilePane === "signals" ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
      ...
      <aside className={`fixed top-[36px] bottom-[48px] md:bottom-[28px] right-0 flex min-h-0 w-full flex-col overflow-hidden md:w-[280px] z-40 bg-white/95 backdrop-blur-md border-l border-[#e4e4e7] transition-transform duration-300 ${mobilePane === "intel" ? "translate-x-0" : "translate-x-full md:translate-x-0"}`}>
      ...
      {showMapResetButton &&
        ...
        >
          <button
            type="button"
            onClick={handleMapChromeResetClick}
          ...
          >
            <Maximize className="w-3.5 h-3.5" /> {mapResetButtonLabel}
          </button>
        </div>
      )}

      <div className="md:hidden fixed bottom-0 left-0 right-0 h-[48px] bg-white border-t border-[#e4e4e7] z-50 flex shadow-lg">
